...
export function classify(document: QualificationDocument, hasMissingAltitude: boolean): Classification {
  ...
  const classification: RouteClassification = document.sourceKind === "waylines-wpml" && document.hasCompanionTemplate
    ? "upload-candidate"
    : "preview-only";
  ...
  if (document.format === "kmz" && document.sourceKind === "waylines-wpml" && !document.hasCompanionTemplate) {
