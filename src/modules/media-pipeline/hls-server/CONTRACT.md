...
`hls-server` 管理一个仅绑定本机回环地址的 HLS 静态文件服务生命周期，并为合法 `streamId` 生成可交给播放器的本地播放地址。文件读取、MIME 类型、HTTP 传输、端口占用和服务进程实现全部由注入适配器负责。

它不启动 FFmpeg、不创建或删除 HLS 片段、不检查播放列表是否就绪、不接收 RTMP、不管理设备或流集合、不播放视频、不读取文件系统或网络配置、不绑定局域网地址。视频就绪与超时仍由 `stream-health` 判定；转码进程仍由 `transcode-runner` 管理。

## 公开接口

```ts
interface HlsServerPort {
  readonly listen: (input: Readonly<{ readonly host: "127.0.0.1"; readonly port: number; readonly rootDirectory: string }>) => void;
  readonly close: () => void;
}

interface HlsServerSnapshot {
...
`create(port)` 要求注入对象同时拥有 `listen` 与 `close` 两个函数，否则同步抛出 `TypeError`。服务启动输入包含 `port` 和 `rootDirectory`：端口为 1024 至 65535 的安全整数；根目录为非空、去除首尾空白后仍非空的字符串。主机固定为 `127.0.0.1`，不是调用者可配置项。
...
6. `playback(streamId)` 只在 `listening` 时有效，返回 `http://127.0.0.1:{port}/hls/{encodeURIComponent(streamId)}/index.m3u8`。它不检查文件是否存在。
...
| 场景 | 诊断 |
| --- | --- |
| 监听失败 | `无法启动本地 HLS 服务。请检查端口与桌面端权限。` |
| 停止失败 | `无法停止本地 HLS 服务。请检查桌面端权限。` |
