...
`video-player` 只把组合根提供的、已经生成的本机 HTTP-FLV `.flv` 播放地址交给播放器适配器，并把播放器报告的致命故障转换为脱敏诊断。它不创建播放器、不读取 DOM、不导入 Electron、不启动或停止 RTMP、转码和 HTTP-FLV 服务，也不负责判断媒体是否已经就绪。
...
- `url`：合法的 `http` 或 `https` URL；不得包含用户名、密码、查询串或片段，路径必须以 `.flv` 结尾。模块不访问该地址，只做格式校验。
