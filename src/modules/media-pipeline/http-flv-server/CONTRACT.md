...
2. `idle` 或 `failed` 可 `start`；调用后先为 `starting`，并且只以 `{ host: "127.0.0.1", port, rootDirectory }` 调用 `listen` 一次。只有适配器 Promise 成功兑现时才进入 `listening` 并返回成功；`listening` 严格表示端口已实际绑定。
3. `listen` 同步抛出或 Promise 拒绝时进入 `failed`，返回 `LISTEN_FAILED` 和稳定诊断；不泄露根目录或原始异常。
4. `starting` 或 `listening` 再次 `start` 返回 `ALREADY_LISTENING`，且不得触及端口适配器。
...
`starting` 与 `listening` 快照保留请求端口；`idle` 与 `failed` 的 `port` 为 `null`。成功、空闲与关闭状态的诊断为 `null`。公开快照、结果和错误文本不得包含 `rootDirectory`、文件名、原始异常、请求地址、令牌或任何局域网接口地址。

## 依赖与验收

本模块是有界异步启动状态机；不能导入 Node、Electron、HTTP 库、文件系统、网络、FFmpeg、UI 或 `media-pipeline` 的其他二级实现。它只保存已验证的端口值，不缓存根目录，也不持有任何文件句柄。端口适配器不得在 `listen` 已调用但尚未绑定时兑现 Promise。
