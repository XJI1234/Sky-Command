...
`device-guidance` 是 `device-console` 中的纯决策二级模块。它只消费已经由 `link-chain` 归一化的单台手机链路快照，给出当前唯一应向操作员展示的常规连接步骤。

它不建立或关闭电脑与手机的连接，不发送配对命令，不修改链路或配对状态，不保存遥测，不读取配置，也不依赖 Electron、Vue、WebSocket、Android、DJI SDK、文件系统或网络库。对频是连接新飞机或更换遥控器时的独立维护操作，不属于常规连接引导；设备就绪只能由已归一化的链路快照确定。
...
4. 遥控器已连接、飞行器状态未知：`WAIT_FOR_SDK`。
5. 遥控器已连接、飞行器明确未连接：`CONNECT_AIRCRAFT`。
6. 三段链路均已连接：`READY`。
...
失败详情只能定位字段和失败类别，不能回显输入、异常消息或内部状态原文。

## 6. 依赖和验证

本模块只能依赖 ECMAScript 基础能力。它不导入 `link-chain` 的内部实现；调用方只传入其公开快照。

测试必须覆盖每个优先级分支、飞行器已明确断开与状态未知、非法和矛盾链路、恶意 getter、不可变性、重复评估、架构隔离、类型边界、性能、全局覆盖率和模块范围 100% 变异测试。
