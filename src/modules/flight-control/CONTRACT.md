...
`action` 只能是 `takeoff`、`land`、`confirm-landing`、`return-home`、`stop-takeoff` 或 `stop-auto-landing`。`request` 永远不发送命令；它只执行预检、能力门禁并创建一份有时限且只可消费一次的确认请求。`confirm` 只能消费同一设备当前待确认请求，成功或失败均关闭该请求。`cancel` 也只能取消同一设备当前待确认请求。
