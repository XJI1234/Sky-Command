...
桌面中继服务器对每条连接保持有界入站背压：超过其 `relay-server` 契约的等待帧上限时，以 `inbound-overflow` 结束该会话，并完全沿普通连接关闭路径删除设备事实、取消本端等待和发布快照。根模块不缓存、重放或跨会话转移这些帧；这只表示本次中继会话无法安全消费入站数据，不表示 DJI 设备或已下发的飞行操作已经停止。

手机诊断批次的桌面持久化是根模块内部的低优先级、有序路径。`RelayDiagnosticSink.persist` 返回 `Promise<boolean>`：只有 Promise 兑现为 `true` 后，模块才可以记录该批次的去重键并发送 `diagnostic-ack`；拒绝或兑现 `false` 时绝不确认。诊断写入等待不得阻塞同一连接的遥测、命令结果、任务帧或连接关闭处理。每个已配对会话同一时刻最多允许一个诊断批次持久化，并最多保留随后八个不同批次；同一未确认批次只保留一次。超过该上限的异常快速上报不确认，手机端依据至少一次投递在前一批结束后重试，而桌面绝不制造无限 Promise 队列。
