...
- `manualHost` 是 `null` 或单个私网 IPv4 地址；不接受域名、协议前缀、端口、
  CIDR、通配符、空白、控制字符或 IPv6。
- IPv4 接受私网地址，拒绝回环。IPv6 不是当前 RTMP/WHIP 推流主机格式，必须拒绝，
  不能猜测方括号规则。公网地址明确拒绝，因为此设置只描述本机局域网连接入口。
- IPv4 去除前导零并规范化为十进制点分格式。
...
- 网络：端口上下界、整数性、IPv4 正规化、拒绝 IPv6/公网/域名/端口/控制字符；
