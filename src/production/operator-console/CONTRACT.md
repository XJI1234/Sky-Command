...
- 飞行页：分别选择任务机与图传机，执行准备航线/上传至飞机/执行航线、旧 RTMP 图传启停、起飞降落返航。起飞必须已确认在地面、电机关闭且已取得不少于 20% 的电量。降落、确认继续降落、返航、停止自动起飞和停止自动降落只要求所选手机在线且 MSDK 已就绪；遥控器、飞控、飞行状态、飞行模式、电量和电机状态只作为显示事实，不能让页面替 MSDK 拒绝收尾操作。飞行页不提供低延迟/WHIP 按钮，也不提供「附着播放器」；图传画面由本页 `flv.js` 播放 HTTP-FLV。设备页和 `control` 都读取同一包遥测中的原始 MSDK Key，按钮门禁继续使用独立的能力门禁与动态遥测，显示不能放宽新操作。飞行页必须在点「启动图传」前展示图传是否可启动（`streamCanStart` / `streamLabel`）；未就绪时禁用启动按钮。图传进行中必须可点「停止图传」（`streamCanStop`），文案要指向停止而不是只说不能启动。
