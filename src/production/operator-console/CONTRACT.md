...
- 设备页：观察手机、遥控器、对频、飞机，以及已确认的机型、遥控器型号、电量、预计可飞时间、飞行状态、电机状态、飞行模式、高度、位置和直播指标。显示顺序必须与操作顺序一致：先连遥控器，再对频，最后才出现飞机。手机未就绪时，遥控器或飞机一律显示「等待手机就绪」，即使收到未连接也不得写成「遥控器未连接」或「飞机未连接」。手机已就绪后：明确断开显示「遥控器未连接」/「飞机未连接」，状态未知显示「遥控器状态未知」/「飞机状态未知」，已连接显示「遥控器已连接」/「飞机已连接」。对频 `UNKNOWN` 必须写成「对频状态未知」，不得写成「未对频」。飞机与飞控不一致时须诚实标出（如「飞机已连上，但飞控未通」）。未知事实必须使用「尚未取得」或完全省略，不能显示 `0`、旧值或推测值。设备页只消费工作流快照，不读取原始中继帧、不发对频、任务或图传命令。
