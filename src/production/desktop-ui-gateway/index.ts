...
function create(options: DesktopUiGatewayOptions): DesktopUiGatewayInstance {
  ...
  return freeze({
    invoke: async (method, input) => {
      ...
      if (method === "flight.request") { const source = exact(input, ["deviceId", "action"]); const deviceId = source === null ? null : read(source, "deviceId"); const action = source === null ? null : read(source, "action"); return !validId(deviceId) || (action !== "takeoff" && action !== "land" && action !== "return-home") ? failure("INVALID_INPUT") : call("requestFlightAction", [deviceId, action]); }
