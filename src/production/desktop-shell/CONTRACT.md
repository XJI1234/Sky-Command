...
`device-refresh` 只映射到 `device.refresh` 并接收 `{ deviceId }`。它是只读状态刷新，不得触发 DJI、图传、任务、直接飞控或任何媒体服务命令。`device-link-measure` 只映射到 `device.link.measure` 并接收 `{ deviceId }`，用于测量既有 WebSocket 协议的往返质量；它不得发送任何 Relay 业务帧、DJI、图传、任务、直接飞控或媒体服务命令，也不得影响按钮门禁。生产外壳不得再登记 `hardware-readiness`。
