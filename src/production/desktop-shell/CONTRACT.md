...
`device-refresh` 只映射到 `device.refresh` 并接收 `{ deviceId }`。它是只读状态刷新，不得触发 DJI、图传、任务、直接飞控或任何媒体服务命令。`hardware-readiness` 只映射到 `hardware.readiness` 并接收 `{ deviceId }`。它是只读预检，不得触发图传、直接飞控或任何媒体服务命令。
