...
| 图传源失效 | 当前显示遥测中 `AirLinkKey.KeyConnection` 或主相机 `CameraKey.KeyConnection(LEFT_OR_MAIN)` 不是 `CONNECTED`；它独立于飞控和 `ProductKey`。 |
...
7. 图传源失效时，本模块必须同时使 RTMP 控制车道和桌面播放器失效。它不得发送自动重启；两项 MSDK Key 后续恢复只允许下一次人工 `startStream` 在手机确认成功后重新放行媒体源。
