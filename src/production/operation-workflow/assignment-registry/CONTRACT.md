...
instance.assign(deviceId, routeId) -> boolean
...
ID 只能由父模块传入已验证的稳定字符串；非法值以 `false` 或 `null` 稳定拒绝。`routesInUse` 与 `snapshot` 返回冻结副本并按设备 ID 排序。分配覆盖仅由父模块在确认对应任务稳定终态后允许；本模块不推断该前置条件。
