...
依赖必须只提供一级模块的公开方法，以及由父模块传入的 `online(deviceId)`、`assignedRoute(deviceId)` 与 `settingsAllowed(deviceId, domain)` 事实。任务操作只映射 `MissionControl` 的六个同名方法；图传只映射 `LiveStreamControl.start/stop`；设置只映射 `DeviceSettingsPanel` 四个方法；直接飞控只映射 `FlightControl.request/confirm/cancel`。飞控请求动作固定为 `takeoff`、`land`、`return-home`、`stop-takeoff`、`stop-auto-landing`，其 DJI Action Key 映射完全属于下游 `flight-control` 契约，工作流不得构造或猜测命令名。这里的共同校验只确认设备在线、输入合法和必要业务顺序；直接飞控请求/确认不要求父工作流硬件快照，`FlightControl` 自己在实际发送前检查最小 Relay/MSDK 可达性。图传与设置的各自模块继续拥有自己的能力门禁。该模块绝不访问中继、协议或 DJI。所有依赖异常统一为稳定失败，且不得泄露内部错误。
