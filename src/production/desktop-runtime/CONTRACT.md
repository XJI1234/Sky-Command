...
`desktop-runtime` 是无界面的桌面端生产装配根。它创建后只负责按固定顺序启动和停止已经构造好的中继、媒体与直播控制模块，汇总可安全展示的只读运行时快照，并在媒体启动失败时回滚已经启动的中继。
...
初始状态为 `idle`。`start()` 固定先调用中继监听，再启动媒体服务；两个步骤都成功后状态变为 `running`。中继启动失败映射为 `RELAY_START_FAILED`；媒体启动失败时必须尝试停止中继，最终映射为 `MEDIA_START_FAILED`。任何底层异常都必须收敛为稳定结果，不能穿过本模块。
