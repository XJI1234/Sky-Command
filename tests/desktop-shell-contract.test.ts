...
describe("桌面外壳 IPC", () => {
  it("只把白名单短名转给网关同名方法，并拒绝未知通道", async () => {
    ...
    await expect(shell.invoke("hardware-readiness", { deviceId: "phone-1" })).resolves.toEqual({ ok: false, code: "METHOD_NOT_ALLOWED" });
    ...
    expect(calls).toEqual(["phone-1", "refresh:phone-1", "measure:phone-1"]);
    ...
    expect(shell.snapshot().ipcMethods).not.toContain("hardware-readiness");
