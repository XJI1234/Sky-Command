...
describe("设备事实摘要", () => {
  it("按稳定顺序格式化已确认设备、飞行和图传事实", async () => {
    ...
    })).toBe("机型 Matrice 4T · 遥控器 DJI RC Plus · 电量 87% · 预计可飞 18分5秒 · 飞机在地面 · 电机未启动 · 飞行模式 N · 高度 12.3 m · 位置 30.27415, 120.15515 · 图传中 · 1920x1080 · 30 fps · 1802 kbps · RTT 42 ms");
  });
  ...
  it("在飞行和图传运行时仅追加可安全表示的边界事实", async () => {
    ...
    })).toBe("电量 0% · 预计可飞 0分0秒 · 飞机在空中 · 电机已启动 · 位置 -90.00000, -180.00000 · 图传中");
  });
