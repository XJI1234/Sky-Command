...
describe("媒体管线 stream-health 契约", () => {
  it("按接收、转码和可播放事件推进单条流，并只在 playback-ready 后报告 ready", () => {
    const health = StreamHealth.create({ ingestTimeoutMs: 1_000, playbackTimeoutMs: 1_000 });
    ...
    expect(health.observe("drone-a", "ingest-started", 20)).toEqual({
      ok: true,
      value: { streamId: "drone-a", revision: 2, state: "awaiting-playback", lastEventAt: 20, diagnostic: null }
    });
    expect(health.observe("drone-a", "transcoder-started", 21)).toEqual({
      ok: true,
      value: { streamId: "drone-a", revision: 3, state: "awaiting-playback", lastEventAt: 21, diagnostic: null }
    });
    expect(health.observe("drone-a", "playback-ready", 22)).toEqual({
      ok: true,
  ...
  it("在输入或可播放等待超时后只产生一次有诊断的停止建议", () => {
    ...
    const playlist = StreamHealth.create({ ingestTimeoutMs: 1_000, playbackTimeoutMs: 1_000 });
    playlist.begin("drone-b", 0);
    playlist.observe("drone-b", "ingest-started", 1);
    expect(playlist.evaluate(1_002)).toEqual({
      ok: true,
      value: {
        snapshots: [{ streamId: "drone-b", revision: 3, state: "failed", lastEventAt: 1, diagnostic: PLAYBACK_TIMEOUT }],
        stopRequests: [{ streamId: "drone-b", diagnostic: PLAYBACK_TIMEOUT }]
      }
    });
  });
  ...
  it("隔离多条流，忽略过期事件，并将异常退出转为单条流的安全失败", () => {
    ...
    health.observe("drone-b", "playback-ready", 2);
    ...
    expect(health.observe("drone-b", "playback-ready", 4)).toEqual({ ok: false, code: "STALE_EVENT" });
    expect(health.evaluate(1_001)).toEqual({
      ok: true,
      value: {
        snapshots: [
  ...
  it("拒绝非法输入而不改状态，并向调用方只交付冻结副本", () => {
    expect(() => StreamHealth.create({ ingestTimeoutMs: 999, playbackTimeoutMs: 1_000 })).toThrow();
    expect(() => StreamHealth.create({ ingestTimeoutMs: "1000", playbackTimeoutMs: 1_000 } as never)).toThrow();
    expect(() => StreamHealth.create({ ingestTimeoutMs: 1_000, playbackTimeoutMs: 999 })).toThrow();
    expect(() => StreamHealth.create({ ingestTimeoutMs: 60_001, playbackTimeoutMs: 1_000 })).toThrow();
    expect(() => StreamHealth.create({ ingestTimeoutMs: 1_000, playbackTimeoutMs: 60_001 })).toThrow();
    expect(() => StreamHealth.create({ ingestTimeoutMs: 60_000, playbackTimeoutMs: 60_000 })).not.toThrow();
    const health = StreamHealth.create({ ingestTimeoutMs: 1_000, playbackTimeoutMs: 1_000 });
    ...
    expect(health.observe("drone-a", "xplayback-ready" as never, 1)).toEqual({ ok: false, code: "INVALID_INPUT" });
    expect(health.observe("drone-a", "playback-ready-now" as never, 1)).toEqual({ ok: false, code: "INVALID_INPUT" });
    expect(health.observe("drone-a", { toString: () => "playback-ready" } as never, 1)).toEqual({ ok: false, code: "INVALID_INPUT" });
  ...
  it("不让旧时刻或不符合当前阶段的事件延后超时，并保持多流快照的稳定排序", () => {
    ...
    expect(health.observe("drone-a", "playback-ready", 2)).toEqual({ ok: false, code: "STALE_EVENT" });
    ...
    expect(health.observe("drone-a", "playback-ready", 4)).toMatchObject({ ok: true });
  ...
  it("以收到输入的时刻作为可播放等待起点，而不是以创建流的时刻计时", () => {
    const health = StreamHealth.create({ ingestTimeoutMs: 1_000, playbackTimeoutMs: 1_000 });
    health.begin("drone-a", 0);
