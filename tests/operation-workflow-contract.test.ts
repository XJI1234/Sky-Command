...
const workflowWith = (overrides: Record<string, unknown> = {}) => OperationWorkflow.create({
  relayOperations: {
    ...
    telemetry: () => ({ payload: { sdkRegistered: true, remoteControllerConnected: true, flightControllerConnected: true, connected: true }, capabilities: {} }),
...
describe("飞行作业工作流模块契约", () => {
  ...
  it("航线上传只使用当前订阅控制快照，不重建 MSDK 状态观察", async () => {
    ...
    const workflow = workflowWith({
      ...
      missionControl: {
        upload: async () => { uploads += 1; return { ok: true }; },
        get: (deviceId: string) => ({ deviceId, phase: "staged" }),
        subscribe: () => () => undefined,
      },
    });
  ...
  it("飞控确认时控制快照暂缺仍委托调度器，并保留下游的可达性结果", async () => {
    ...
    const confirmation = { deviceId: "relay-a", action: "takeoff", confirmationId: "confirm-a" };
    ...
    const workflow = workflowWith({
      relayOperations: {
        ...
        telemetry: () => ({
          ...
          payload: { sdkRegistered: true, remoteController: "CONNECTED", flightController: "CONNECTED", aircraft: "CONNECTED", remoteControllerConnected: true, flightControllerConnected: true, connected: true },
          capabilities: {},
        }),
        controlTelemetry: () => ({
          payload: { sdkRegistered: true, remoteController: "CONNECTED", flightController: "CONNECTED", aircraft: "CONNECTED", remoteControllerConnected: true, flightControllerConnected: true, connected: true },
  ...
  it("设备页立即显示本次 MSDK 连接事实，但可达 MSDK 的飞行与设置请求不由桌面遥测提前拒绝", async () => {
    ...
    const workflow = workflowWith({
      ...
      relayOperations: {
        ...
        controlTelemetry: () => ({ payload, capabilities: { liveVideo: true } }),
        refreshTelemetry: async () => ({ status: "succeeded" }),
        subscribe: () => () => undefined,
      },
      liveStreamControl: {
  ...
  it("只投影在线设备，并在设备断连时清除分配和取消尚未确认的飞控动作", async () => {
    ...
    const workflow = OperationWorkflow.create({
      relayOperations: {
        ...
        telemetry: () => ({ payload: { sdkRegistered: true, remoteController: "CONNECTED", flightController: "CONNECTED", aircraft: "CONNECTED", remoteControllerConnected: true, flightControllerConnected: true, connected: true, batteryPercent: 80, isFlying: false }, capabilities: { waypointMission: true, waypointMissionSupport: "supported", liveVideo: true } }),
        controlTelemetry: () => ({ payload: { sdkRegistered: true, remoteController: "CONNECTED", flightController: "CONNECTED", aircraft: "CONNECTED", remoteControllerConnected: true, flightControllerConnected: true, connected: true, batteryPercent: 80, isFlying: false }, capabilities: { waypointMission: true, waypointMissionSupport: "supported", liveVideo: true } }),
        refreshTelemetry: async () => ({ status: "succeeded" }),
  ...
  it("只通过航线库选择航线，且不替换已经分配给设备的航线", () => {
    ...
    const workflow = OperationWorkflow.create({
      relayOperations: { devices: () => [{ deviceId: "relay-a" }], telemetry: () => ({ payload: { sdkRegistered: true, remoteControllerConnected: true, flightControllerConnected: true, connected: true }, capabilities: {} }), controlTelemetry: () => ({ payload: { sdkRegistered: true, remoteControllerConnected: true, flightControllerConnected: true, connected: true }, capabilities: {} }), refreshTelemetry: async () => ({ status: "succeeded" }), subscribe: () => () => undefined },
  ...
  it("旧图传不依赖遥控器或飞控，但必须把实时图传能力交给图传控制模块", async () => {
    ...
    const workflow = workflowWith({
      ...
      liveStreamControl: {
        start: async (deviceId: string) => { started.push(deviceId); return { ok: true }; },
        stop: async () => ({ ok: true }),
        get: () => ({ phase: "idle" }),
        list: () => [],
        recordDisconnected: () => undefined,
  ...
  it("飞控链路未就绪时仍把确认请求交给飞控模块，由 MSDK 的 Action 回调裁决", async () => {
    ...
    const workflow = workflowWith({
      relayOperations: {
        ...
        controlTelemetry: () => ({ payload: { sdkRegistered: true, remoteControllerConnected: true, flightControllerConnected: false, connected: false } }),
        refreshTelemetry: async () => ({ status: "succeeded" }),
        subscribe: () => () => undefined,
      },
      flightControl: {
  ...
  it("在媒体、时钟和播放器依赖异常时不泄露异常且可在释放后拒绝", async () => {
    ...
    await expect(faulty.requestFlightAction("relay-a", "takeoff")).resolves.toMatchObject({ ok: false, code: "DISPOSED" });
  ...
  it("同一手机换了会话时必须复位图传车道并作废危险确认，不得继续显示已启动", async () => {
